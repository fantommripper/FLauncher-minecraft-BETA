...
class APP(CTk.CTk):
    def __init__(self):
        ...
        self.config.read(d.config_file_path)  # Читаємо файл з новим ім'ям
        if 'Player' in self.config:
            saved_nickname = self.config['Player'].get('nickname', '')
            d.nickname = saved_nickname
        ...
        self.nicknameEntry.insert(0, d.nickname)
    ...
    def openSetting(self):
        ...
        #     self.settingswindow.lift()
        #     self.after(100, lambda: self.settingswindow.focus_force())
        messagebox.showerror("ЕТО BETA", "здесь пока ничего нет")
